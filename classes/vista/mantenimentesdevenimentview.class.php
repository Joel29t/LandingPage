...
class MantenimentEsdevenimentView
{

    public function showMantenimentEsdeveniment($ObjectArray, $data = null, $errors = null, $updateData = null, $isCreate = true)
    {
    ...
    {
        if ($isCreate) {
            $table = '<form method="post" action="?MantenimentEsdeveniment/create" enctype="multipart/form-data">';
        } else {
            $table = '<form method="post" action="?MantenimentEsdeveniment/update" enctype="multipart/form-data">';
        }
        $table .= '<table>
        <thead>
            <tr>';
    ...
    <tbody>
        ...
        foreach ($objectArray[0] as $key => $value) {
            ...
            if ($key === 'id') {
                ...
                $table .= '<input type="file" id="' . $key . '" name="' . $key . '"';
            } else {
                $table .= '<input type="text" id="' . $key . '" name="' . $key . '"';
            }
            ...
            if (isset($updateData[$key]) && $key !== 'imatge') {
        ...
        foreach ($objectArray as $result) {
            ...
            foreach ($result as $key => $value) {
                if ($key === 'imatge') {
                    $table .= '<td><img src="' . $value . '" alt="Imatge" style="max-width: 300px;"></td>';
                } else {
                    $table .= '<td>' . $value . '</td>';
                }
            }
        ...
        <div class="button-container">
            <a href="?MantenimentEsdeveniment/showUpdate&id=' . $result->id . '">Actualizar</a>
          <a href="?MantenimentEsdeveniment/delete&id=' . $result->id . '" class="delete-link" data-id="' . $result->id . '">Eliminar</a>
        </div>
    </td>
</tr>';
        }
